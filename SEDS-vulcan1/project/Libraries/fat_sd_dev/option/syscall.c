/*------------------------------------------------------------------------*/
/* Sample code of OS dependent controls for FatFs R0.08.dev               */
/* (C)ChaN, 2010                                                          */
/*------------------------------------------------------------------------*/
...
#if _USE_LFN == 3	/* LFN with working buffer on the heap */
/*------------------------------------------------------------------------*/
...
{
	return HeapAlloc(GetProcessHeap(), 0, size);	// Win32
//	return malloc(size);							// ANSI
}
...
void ff_memfree(
	void* mblock	/* Pointer to the memory block to be free */
)
{
	HeapFree(GetProcessHeap(), 0, mblock);	// Win32
//	free(mblock);							// ANSI
